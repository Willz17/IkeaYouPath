...
const NewList = () => {
  ...
  const CART_URL = "http://localhost:4000/api/users/cart";
  ...
  const addItemToCart = (event, index) => {
    console.log(index);

    const request_data = {
      u_email: JSON.parse(localStorage.getItem('cred')).email,
      u_ID: JSON.parse(localStorage.getItem('cred')).userID, 
      p_ID: productList[index].id, 
      Q: 1,
    }

    console.log(request_data);
    axios.post(CART_URL, request_data, { mode: "no-cors"}).then(() => {
        ...
        <Row className="align-items-center">
          ...
          <div className="col-md-3">
            <Button
              ...
              type="submit"
              size="sm"
              onClick={(e) => { addItemToCart(e, itemId[index]);
              }}
            >
              <img
                src="addBasket.png"
